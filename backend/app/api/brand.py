...
def _company_assets_collection():
...
async def upload_brand_image(
    ...
    _company_assets_collection().update_one(
...
@router.get("/image")
async def get_brand_image(request: Request):
    """Serve the current company image."""
    asset = _company_assets_collection().find_one({"asset_key": COMPANY_ASSET_KEY})
