...
from app.data.repository_providers import get_auth_repository
...
def _initiate_email_verification(
    repo: AuthRepository,
    username: str,
    email: str,
    path: str,
    ...
    now = datetime.utcnow()
    repo.delete_pending_email_verifications(username=username, now=now)
...
async def register_send_otp(request: OTPSendRequest, auth_repo: AuthRepository = Depends(get_auth_repository)):
    """
    ...
    if auth_repo.find_user_by_username_or_email(username=request.username, email=request.email):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
...
async def register_verify_otp(request: OTPVerifyRegisterRequest, auth_repo: AuthRepository = Depends(get_auth_repository)):
    """
    ...
    user_dict = auth_repo.create_user(user_dict)
...
async def register(user: UserCreate, auth_repo: AuthRepository = Depends(get_auth_repository)):
    """
    ...
    if auth_repo.find_user_by_username_or_email(username=user.username, email=user.email):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="Username or email already registered")
...
async def login(
    ...
    * Within that session a JWT is created **only** if no active (non-expired)
    ...
    user = auth_repo.find_user_by_username(user_credentials.username)
...
async def logout(
    ...
    if session_id:
        session = auth_repo.find_session_by_id(session_id)
        username = session["username"] if session else session_id
        auth_repo.delete_session_by_id(session_id)
...
async def get_profile(
    ...
    user = auth_repo.find_user_by_username(current_username, projection={"password_hash": 0})
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
...
async def update_profile(
    ...
    user = auth_repo.find_user_by_username(current_username)
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    ...
    if requested_email and requested_email != user.get("email"):
        existing_with_email = auth_repo.find_user_by_email_excluding_username(
    ...
    auth_repo.update_user_by_username(current_username, updates)

    updated = auth_repo.find_user_by_username(current_username, projection={"password_hash": 0})
    return {
...
async def confirm_email_verification(
    ...
    # Verify token
    verification_record = auth_repo.find_valid_email_verification(
        username=current_username,
        token=token,
        now=datetime.utcnow(),
    )
    ...
    now = datetime.utcnow()
    auth_repo.update_user_by_username(current_username, {"email_verified": True, "updated_at": now})
    auth_repo.mark_email_verification_verified(verification_record["_id"], verified_at=now)
...
async def change_phone_send_otp(
    ...
    if auth_repo.find_user_by_phone_excluding_username(phone=request.new_phone, username=current_username):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
...
async def change_phone_verify_otp(
    ...
    auth_repo.update_user_by_username(
        current_username,
        {
            "phone": request.new_phone,
            "phone_verified": True,
...
async def change_password(
    ...
    auth_repo.update_user_by_username(
        current_username,
        {"password_hash": hash_password(req.new_password), "updated_at": datetime.utcnow()},
    )
