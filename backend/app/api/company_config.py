...
def load_company_config_into_settings():
    ...
    doc = _col().find_one({"configId": "msg91"})
    if doc:
    ...
    misc_doc = _col().find_one({"configId": "misc"})
    if misc_doc:
...
async def get_msg91_config(_: dict = Depends(require_role(["admin"]))):
...
async def get_misc_config(_: dict = Depends(require_role(["admin"]))):
    """Return miscellaneous company settings (admin only)."""
    doc = _col().find_one({"configId": "misc"})
    if doc:
