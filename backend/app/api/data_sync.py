...
def _upsert_entity_row(entity: str, normalized: dict[str, Any]) -> None:
    ...
    now = _now()
    existing = repo.find_one(collection_name, flt)
    ...
    if existing:
        repo.update_one(collection_name, flt, normalized)
        return

    normalized["created_at"] = now
    repo.insert_one(collection_name, normalized)
...
def _create_job(entity: str, job_type: str, requested_by: str, source_filename: str | None = None) -> str:
    ...
    stored = get_order_cart_repository().insert_one(JOB_COLLECTION, doc)
    return stored.get("id", "")


def _mark_job_started(job_id: str) -> None:
    get_order_cart_repository().update_one(
        JOB_COLLECTION,
        {"id": job_id},
...
def _mark_job_finished(job_id: str, *, processed: int, success: int, failed: int, errors: list[dict[str, Any]], result: dict[str, Any] | None = None) -> None:
...
def _mark_job_failed(job_id: str, detail: str) -> None:
    get_order_cart_repository().update_one(
        JOB_COLLECTION,
        {"id": job_id},
        {
...
def _run_import_job(job_id: str, entity: str, csv_text: str) -> None:
    try:
        ...
        _mark_job_finished(job_id, processed=processed, success=success, failed=failed, errors=errors)
    except Exception as exc:
...
def _run_export_job(job_id: str, entity: str) -> None:
    try:
        ...
        docs = repo.find_many(cfg["collection"], query)
...
async def list_jobs(
    ...
    repo = get_order_cart_repository()
    docs = repo.find_many(JOB_COLLECTION, {})
    # Sort by created_at desc, apply skip/limit in Python (small collection)
    docs.sort(key=lambda d: d.get("created_at", datetime.min), reverse=True)
    return [_sanitize_doc(d) for d in docs[skip:skip + limit]]


@admin_router.get("/jobs/{job_id}")
async def get_job(job_id: str, _: dict = Depends(require_role(["admin"]))):
    doc = get_order_cart_repository().find_one(JOB_COLLECTION, {"id": job_id})
    if not doc:
...
async def download_export(job_id: str, _: dict = Depends(require_role(["admin"]))):
    ...
    filename = f"{doc.get('entity', 'entity')}_export_{doc.get('id', job_id)}.csv"
