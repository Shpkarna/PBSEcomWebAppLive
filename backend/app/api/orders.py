...
async def list_orders(
    ...
    orders = repo.list_user_orders(_customer_keys(user), skip, limit)
...
async def update_order_status(
    ...
    matched = repo.update_order(order_id, {"status": new_status, "updated_at": datetime.utcnow()})
    if matched == 0:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Order not found")
...
async def return_order(
    ...
    for item in order.get("items", []):
        try:
            repo.increment_product_stock(item["product_id"], item["quantity"])
    ...
    repo.update_order(order_id, {"status": "returned", "return_reason": body.reason, "updated_at": datetime.utcnow()})
    updated = repo.find_order_by_id(order_id)
...
async def exchange_order(
    ...
    new_product = repo.find_product_by_id(body.new_product_id)
    if not new_product:
    ...
    repo.increment_product_stock(body.new_product_id, -body.quantity)
    repo.insert_stock_ledger_row({
    ...
    if not customer_business_id:
        ...
        repo.set_customer_business_id(user["_id"], customer_business_id)
    ...
    new_order = repo.create_order(exchange_order_doc)
