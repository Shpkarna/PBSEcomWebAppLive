...
def ensure_default_role_mappings() -> None:
    ...
    role_permissions = get_collection("role_permissions")
    role_func_mappings = get_collection("role_functionality_mappings")

    for role, functionalities in DEFAULT_ROLE_MAPPINGS.items():
        role_permissions.update_one(
            {"_id": role},
            {"$setOnInsert": {"functionalities": functionalities}},
            upsert=True,
        )
        for code in functionalities:
            role_func_mappings.update_one(
                {"role": role, "functionality_code": code},
...
def get_role_mappings() -> Dict[str, List[str]]:
    ...
    for doc in role_permissions.find({}):
...
def get_user_role(username: str) -> str:
    ...
    user_role_mappings = get_collection("user_role_mappings")
    mapped = user_role_mappings.find_one({"username": username})
