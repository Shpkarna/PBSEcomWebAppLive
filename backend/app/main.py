...
from app.data.driver_registry import resolve_shutdown, current_engine
from app.data.repository_providers import get_database_bootstrap
...
async def startup_event():
    ...
    try:
        print(f"Starting with database engine: {current_engine()}")
        get_database_bootstrap().bootstrap()
...
async def shutdown_event():
    """Close database connection on shutdown (engine-agnostic)."""
    resolve_shutdown()()
...
# Observability metrics endpoint — Phase 9 criterion 9.3
@app.get("/api/metrics")
async def get_metrics():
    """Return per-repository latency and failure counters collected since process start.

    Returns a JSON summary suitable for scraping by an external monitoring tool.
    No authentication is required (internal/ops endpoint — restrict at the network
    layer in production).
    """
