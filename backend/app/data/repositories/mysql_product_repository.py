"""MySQL-backed product repository implementation."""
...
class MySQLProductRepository(MySQLRepositoryBase, ProductRepository):
    """MySQL implementation for product persistence and media storage."""
    ...
    def list_products(
        ...
        if category:
            rows = self._execute(
                (
                    f"SELECT * FROM products WHERE LOWER(category) = LOWER(%s) "
                    f"ORDER BY {sort_key} {'DESC' if sort_dir < 0 else 'ASC'} LIMIT %s OFFSET %s"
                ),
                [category, int(limit), int(skip)],
                fetchall=True,
            )
