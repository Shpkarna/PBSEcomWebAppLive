"""SQL Server-backed cart/order gateway repository (Phase 7)."""
...
class MSSQLOrderCartRepository(MSSQLRepositoryBase, OrderCartRepository):
    """SQL Server implementation for cart, order, admin, and config persistence."""
    ...
    def backfill_cart_quote_id(self, user_id: str, session_id: str, cart_quote_id: str) -> None:
        self._execute(
            (
                "UPDATE cart_items SET cart_quote_id = ? "
    ...
    def acquire_checkout_lock(self, cart_quote_id: str, session_id: str, ttl_minutes: int) -> None:
        ...
        with mssql_transaction():
            with mssql_connection() as connection:
                ...
                # WITH (UPDLOCK, ROWLOCK) is the T-SQL equivalent of MySQL FOR UPDATE
                cursor.execute(
                    (
                        "SELECT cart_quote_id, session_id, expires_at FROM checkout_locks "
                        "WITH (UPDLOCK, ROWLOCK) WHERE cart_quote_id = ?"
                    ),
                    [cart_quote_id],
                )
                row = cursor.fetchone()
                if row is None:
                    cursor.execute(
                        (
                            ...
                            "(cart_quote_id, session_id, expires_at, created_at, updated_at, payload_json) "
                            "VALUES (?, ?, ?, ?, ?, NULL)"
                        ),
                ...
                if row_dict.get("session_id") == session_id or row_dict.get("expires_at") < now:
                    cursor.execute(
                        (
                            "UPDATE checkout_locks SET session_id = ?, expires_at = ?, updated_at = ? "
                            "WHERE cart_quote_id = ?"
                        ),
    ...
    def aggregate_orders(self, pipeline: list) -> list[dict]:
        raise NotImplementedError("aggregate_orders is not used by the current Phase 7 runtime")
    ...
    def search_sales_orders(
        ...
        *,
        ...
        if order_id:
            pattern = f"%{order_id}%"
            conditions.append("(LOWER(id) LIKE LOWER(?) OR LOWER(order_number) LIKE LOWER(?))")
            params.extend([pattern, pattern])
        if order_number:
            conditions.append("LOWER(order_number) LIKE LOWER(?)")
