...
class MySQLRepositoryBase:
    """Common SQL and row-mapping helpers used by MySQL repositories."""
    ...
    def _spec(self, collection_name: str) -> TableSpec:
        try:
            return MYSQL_TABLE_SPECS[collection_name]
        except KeyError as exc:
            raise ValueError(f"Unsupported MySQL collection '{collection_name}'") from exc
            ...
            if isinstance(value, dict):
                for operator, operand in value.items():
                    if operator == "$ne":
                        ...
                        clauses.append(f"LOWER({column}) LIKE LOWER(%s)")
    ...
    def _find_one_row(self, collection_name: str, filter_doc: dict) -> Optional[dict]:
        ...
        return self._execute(
            f"SELECT * FROM {spec.table_name} WHERE {where_sql} LIMIT 1",
            params,
            fetchone=True,
        )
    ...
    def _find_rows(
        ...
        *,
        ...
        spec = self._spec(collection_name)
        where_sql, params = self._build_where(spec, filter_doc)
        sql = f"SELECT * FROM {spec.table_name} WHERE {where_sql}"
        if order_by:
            sql += f" ORDER BY {order_by} {'DESC' if descending else 'ASC'}"
        if limit is not None:
            sql += " LIMIT %s OFFSET %s"
            params = [*params, int(limit), int(skip)]
        elif skip:
    ...
    def _update_doc(self, collection_name: str, filter_doc: dict, updates: dict, *, upsert: bool = False) -> int:
        ...
        if spec.json_column:
            ...
            assignments.append(f"{spec.json_column} = %s")
            values.append(dumps_json(payload))
