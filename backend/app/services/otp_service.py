...
class OTPService:
    ...
    def create_otp_record(phone: str, purpose: str = "registration") -> dict:
        ...
        repo.delete_unverified_otp_records(phone, purpose)
        ...
        repo.create_otp_record(otp_record)
    ...
    def verify_otp(phone: str, otp: str, purpose: str = "registration") -> dict:
        ...
        otp_record = repo.find_active_otp(phone, purpose, now)
        ...
        if otp_record["otp"] != otp:
            # Increment attempts
            repo.increment_otp_attempts(otp_record["_id"])
        ...
        repo.mark_otp_verified(otp_record["_id"], datetime.now(timezone.utc))
    ...
    def get_verified_otp_record(phone: str, purpose: str = "registration") -> dict:
        ...
        """
        repo = get_utility_repository()
        since = datetime.now(timezone.utc) - timedelta(minutes=OTPService.OTP_EXPIRY_MINUTES)
        return repo.find_verified_otp(phone, purpose, since)
    
    @staticmethod
    def mark_otp_used(otp_record_id: str):
        """Mark an OTP record as used (after successful registration/phone change)."""
        repo = get_utility_repository()
        repo.mark_otp_used(otp_record_id, datetime.now(timezone.utc))
