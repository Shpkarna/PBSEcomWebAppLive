...
def run_cart_cleanup() -> dict:
    ...
    session_ids_in_cart = cart_col.distinct("session_id")
    ...
    for sid in session_ids_in_cart:
        session = sessions_col.find_one({"_id": sid})
    ...
    for sid in dead_session_ids:
        ...
        orphaned_items = list(cart_col.find({"session_id": sid}))
        ...
        if quote_ids:
            lock_result = locks_col.delete_many({"cart_quote_id": {"$in": quote_ids}})
            total_locks_removed += lock_result.deleted_count

        # Remove the cart items
        cart_result = cart_col.delete_many({"session_id": sid})
        total_cart_items_removed += cart_result.deleted_count
        ...
        log_event(
            ...
            details={
                ...
                "cart_items_removed": cart_result.deleted_count,
                "locks_removed": lock_result.deleted_count if quote_ids else 0,
